...
namespace :hiveway do
  ...
  task :setup do
    ...
    begin
      ...
      using_docker        = prompt.yes?('Are you using Docker to run etherhive?')
      ...
      loop do
        ...
        env['DB_NAME'] = prompt.ask('Name of PostgreSQL database:') do |q|
          q.required true
          q.default using_docker ? 'postgres' : 'etherhive_production'
          q.modify :strip
        end

        env['DB_USER'] = prompt.ask('Name of PostgreSQL user:') do |q|
          q.required true
          q.default using_docker ? 'postgres' : 'etherhive'
          q.modify :strip
        end
      ...
      loop do
        ...
        env['SMTP_FROM_ADDRESS'] = prompt.ask('E-mail address to send e-mails "from":') do |q|
          q.required true
          q.default "etherhive <notifications@#{env['LOCAL_DOMAIN']}>"
          q.modify :strip
        end
        ...
        begin
          ...
          mail = ActionMailer::Base.new.mail to: send_to, subject: 'Test', body: 'etherhive SMTP configuration works!'
          mail.deliver
        rescue StandardError => e
      ...
      if prompt.yes?('Save configuration?')
        ...
        File.write(Rails.root.join('.env.production'), "# Generated with etherhive:setup on #{Time.now.utc}\n\n" + env.each_pair.map { |key, value| "#{key}=#{value}" }.join("\n") + "\n")
        ...
        prompt.say "\n"
        prompt.ok 'All done! You can now power on the etherhive server 🐘'
        prompt.say "\n"
  ...
  namespace :maintenance do
    ...
    task purge_removed_accounts: :environment do
      ...
      OptionParser.new do |opts|
        opts.banner = 'Usage: rails etherhive:maintenance:purge_removed_accounts [options]'
